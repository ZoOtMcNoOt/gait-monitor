...
jest.mock('../ScrollableContainer', () => {
  return function MockScrollableContainer({ children, id, className }: { children: React.ReactNode; id: string; className: string }) {
...
describe('ConnectTab', () => {
  ...
  test('renders description text', async () => {
    ...
    expect(container.textContent).toContain('Scan for and connect to Bluetooth devices for data collection.');
  });
