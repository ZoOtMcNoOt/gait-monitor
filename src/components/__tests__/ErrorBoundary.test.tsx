...
describe('ErrorBoundary', () => {
  ...
  const renderWithErrorBoundary = (children: React.ReactElement, fallback?: React.ReactNode) => {
    try {
      flushSync(() => {
        root.render(React.createElement(ErrorBoundary, { children, fallback }))
      })
    } catch {
  ...
  test('renders children when no error occurs', async () => {
    await renderWithErrorBoundary(React.createElement(ThrowError, { shouldThrow: false }))

    expect(container.textContent).toContain('No error')
  })

  test('renders default error UI when error occurs', async () => {
    await renderWithErrorBoundary(React.createElement(ThrowError, { shouldThrow: true }))
