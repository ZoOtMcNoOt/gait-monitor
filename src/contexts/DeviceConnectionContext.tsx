...
interface DeviceConnectionState {
  ...
  deviceSampleRates: Map<string, number>
...
export const DeviceConnectionProvider: React.FC<DeviceConnectionProviderProps> = ({ children }) => {
  ...
  const startDeviceCollection = useCallback(async (deviceId: string) => {
    try {
      ...
      setActiveCollectingDevices((prev) => (prev.includes(deviceId) ? prev : [...prev, deviceId]))
        ...
        availableDevices.forEach((deviceId) => {
          ...
          if (!isActuallyConnected) {
            ...
            if (lastGait && now - lastGait < GAIT_DATA_TIMEOUT) {
              ...
              console.warn(
                `[BLE][Warn] Device ${deviceId}: BLE connected but no gait data for ${Math.round((now - lastGait) / 1000)}s`,
              )
            } else {
  ...
  // Actions - Sample Rate
  const getCurrentSampleRate = useCallback(
