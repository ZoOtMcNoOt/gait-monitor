...
describe('CSRF Protection Service', () => {
  ...
  describe('Rate Limiting Integration', () => {
    test('should handle rate limit exceeded error from backend', async () => {
      mockInvoke
        ...
        .mockRejectedValue(
          new Error('Rate limit exceeded for file operations. Please wait before trying again.'),
        )
    ...
    test('should handle CSRF token validation failure from backend', async () => {
      mockInvoke
        .mockResolvedValueOnce('test-token') // get_csrf_token (initialize)
        .mockRejectedValue(
          new Error('Invalid or expired CSRF token. Please refresh and try again.'),
        )
  ...
  describe('Performance and Reliability', () => {
    ...
    test('should maintain security under load', async () => {
      ...
      mockInvoke.mockImplementation(async (command) => {
        ...
        switch (command) {
          ...
          case 'save_filtered_data':
            throw new Error(
              'Rate limit exceeded for file operations. Please wait before trying again.',
            )
      ...
      const operations = [
        ...
        withCSRFProtection(async (token) =>
          mockInvoke('copy_file_to_downloads', { csrfToken: token }),
        ),
        withCSRFProtection(async (token) =>
          mockInvoke('choose_storage_directory', { csrfToken: token }),
        ),
      ]
  ...
  describe('Token Management', () => {
    ...
    test('should refresh token when requested', async () => {
      ...
      mockInvoke.mockResolvedValueOnce(originalToken).mockResolvedValueOnce(newToken)
  ...
  describe('Rate Limiting', () => {
    test('should handle rate limit exceeded error', async () => {
      ...
      await expect(protectedOperations.saveSessionData('test', 'test', 'test', [])).rejects.toThrow(
    ...
    test('should handle CSRF token validation failure', async () => {
      ...
      mockInvoke
        ...
        .mockRejectedValueOnce(
          new Error('Invalid or expired CSRF token. Please refresh and try again.'),
        ) // refresh_csrf_token also fails

      await expect(protectedOperations.deleteSession('test-session')).rejects.toThrow(
