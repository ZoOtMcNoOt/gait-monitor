...
BLECharacteristic gaitCharacteristic(
  ...
  BLERead   | BLENotify,
  29,       // 5-byte timestamp + 6 floats (24 bytes)
  true      // fixed length
);
...
// [5..28] 6 floats (24 bytes) R1,R2,R3,X,Y,Z
...
void setup() {
  ...
  // Increase connection interval for power savings while maintaining <50ms latency
  BLE.setConnectionInterval(30, 45); // min 30ms, max 45ms
...
void loop() {
  ...
  while (central.connected()) {
    ...
    if ((long)(nowUs - lastSampleUs) >= (long)SAMPLE_INTERVAL_US) {
      ...
      // High-resolution device timestamp (microseconds) using micros()
      // We take lower 40 bits (5 bytes). This gives ~12.7 days range at 1 µs resolution.
      // If multi-week continuous operation is required, extend range or send periodic epoch sync.
      unsigned long nowMicros = micros(); // 32-bit on many Arduinos (rollover ~70 mins)
      static unsigned long lastMicros = 0;
      static uint32_t rolloverCount = 0; // counts 32-bit micros() rollovers
      if (nowMicros < lastMicros) { // rollover detected
        rolloverCount++;
      }
      lastMicros = nowMicros;
      // Build 64-bit extended counter
      unsigned long long extended = ((unsigned long long)rolloverCount << 32) | (unsigned long long)nowMicros; // still increments every µs
      unsigned long long ts40 = extended & 0xFFFFFFFFFFULL; // lower 40 bits
      // Write 5-byte little endian timestamp
      for (int i = 0; i < 5; i++) {
        data_packet[i] = (uint8_t)((ts40 >> (8 * i)) & 0xFF);
      }
      // Copy floats after timestamp
